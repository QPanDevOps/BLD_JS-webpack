...
Entrypoints that depend on other entrypoints do not need their own runtime.
They will use the runtime(s) from referenced entrypoints instead.
Remote the 'runtime' option from the entrypoint.`);
...
Did you mean to use 'dependOn: ${JSON.stringify(
							runtime
						)}' instead to allow using ${name} within the runtime of ${runtime}? For this ${runtime} must always be loaded when ${name} is used.
Or do you want to use ${name} and ${runtime} independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option, but do not use a name of another entrypoint.`);
						err.chunk = entry.getEntrypointChunk();
						this.errors.push(err);
						continue;
					}
